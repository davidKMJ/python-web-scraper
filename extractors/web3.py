...
def extract_web3_jobs(keyword):
    ...
    response = requests.get(
        f"https://web3.career/{keyword}-jobs", headers={"User-Agent": user_agent}
    )
    soup = BeautifulSoup(response.text, "html.parser")
    jobs = soup.find("table", class_="table").find_all("tr", class_="table_row")

    for job in jobs:
        if job.has_attr("id") and "sponsor" in job["id"]:
            continue
        company = job.find("h3").text.strip()
        position = job.find("h2", class_="fs-6 fs-md-5 fw-bold my-primary").text.strip()
